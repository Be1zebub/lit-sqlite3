...
local function prepare(conn, query, args)
	local stmt = conn:prepare(query)

	if args and #args > 0 then
		for i = 1, #args do
			stmt:_bind1(i, args[i])
		end
	end
...
function conn_mt:Query(query, args, opts)
	...
	elseif opts.value then
		local row = stmt:QueryRow() or {}

		if opts.value ~= true then
			return row[opts.value]
		end

		local _, value = next(row)
		return value
	else
		sql.sqlite3_step(stmt._ptr) -- just commit
		if opts.insert then
			local row = prepare(self, "SELECT LAST_INSERT_ROWID() AS `insert_id`;"):QueryRow()
			return row and row.insert_id
		end
	end
end
...
function stmt_mt:QueryRow()
	if self._code == sql.SQLITE_DONE then return end
	self._code = sql.sqlite3_step(self._ptr)
	...
	if self._code == sql.SQLITE_ROW then
		...
		for i = 0, self:_ncol() -1 do
			row[ffi.string(sql.sqlite3_column_name(self._ptr, i))] = get_column(self._ptr, i)
		end

		return row
	elseif self._code ~= sql.SQLITE_DONE then
		E_conn(self._conn, self._code)
	end
end

function stmt_mt:QueryAll()
	local response, row = {}
